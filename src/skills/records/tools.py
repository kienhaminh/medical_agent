...
def search_records_by_content(
    patient_id: int,
    search_term: str,
    limit: int = 5
) -> str:
    """Search medical records by content keywords.
    
    Use this tool when you need to find specific records containing certain keywords.
    This performs a simple text search rather than semantic search.

    Args:
        patient_id: The patient's ID.
        search_term: Keyword to search for in record content.
        limit: Maximum number of records to return.

    Returns:
        Matching medical records.
    """
    logger.info("Searching records for patient_id=%s, term='%s'", patient_id, search_term)
    ...
    try:
        stmt = (
            select(MedicalRecord)
            .where(MedicalRecord.patient_id == patient_id)
            .where(MedicalRecord.content.ilike(f"%{search_term}%"))
            .order_by(MedicalRecord.created_at.desc())
            .limit(limit)
        )
        ...
        response = [f"Records containing '{search_term}' for Patient ID {patient_id}:"]
        for r in records:
            created_str = r.created_at.strftime("%Y-%m-%d")
            response.append(f"  - [{created_str}] {r.record_type.upper()}: {r.content[:100]}...")
        
        return "\n".join(response)
        
    except Exception as e:
        logger.exception("Error searching records: %s", e)
