"""Tools for patient-management skill."""
...
def list_all_patients(limit: int = 50) -> str:
    """List all patients in the system.
    
    Use this tool when you need to see a list of all patients or
    ...
    Returns:
        A formatted string containing the list of patients.
    """
    limit = min(limit, 100)
    
    session: Session = SessionLocal()
    try:
        stmt = select(Patient).order_by(Patient.id).limit(limit)
        patients = session.execute(stmt).scalars().all()
        
        if not patients:
            return "No patients found in the system."
        
        response = [f"Total patients: {len(patients)} (showing up to {limit})"]
        response.append("")
        ...
        logger.exception("Error listing patients: %s", e)
        return f"Error listing patients: {str(e)}"
    finally:
        session.close()
