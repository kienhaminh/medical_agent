...
class GraphBuilder:
    ...
    def build(self):
        ...
        async def delegate_to_specialist(specialist_name: str, query: str) -> str:
            ...
            """
            logger.info("[DELEGATION] Delegating to %s with query: %s", specialist_name, query)
            ...
            try:
                ...
                if responses:
                    ...
                    logger.debug("[Main Agent] Received response from %s: %s...", specialist_name, final_response.content[:500])
